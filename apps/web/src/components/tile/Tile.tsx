...
export function Tile({ title, variant = 'poster', hoverPreview = true }: Props) {
  ...
  function onEnter() {
    ...
    timer.current = window.setTimeout(async () => {
      setExpanded(true);
      // pick the first file (movie: only file; series: first episode)
      try {
  ...
  return (
    ...
    >
      <motion.div
        layout="position"
        className={`relative ${aspect} rounded-md overflow-hidden bg-neutral-900 shadow-lg`}
        animate={{ scale: expanded ? 1.18 : 1, zIndex: expanded ? 30 : 1 }}
          ...
          {expanded ? (
            ...
            >
              ...
              <div className="mt-1 flex items-center gap-2 text-[10px] uppercase tracking-wider text-neutral-300">
                ...
                {title.kind === 'series' ? (
                  <span>{title.season_count} S · {title.episode_count} E</span>
                ) : null}
